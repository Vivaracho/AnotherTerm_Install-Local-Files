...
echo '  Arch:' "$MY_DEVICE_ABIS" '   *may vary'
echo '  Api:' "$MY_ANDROID_SDK" '   Variant: '"$VARIANT"
echo
echo '  minitar:  https://github.com/green-green-avk/build-libarchive-minitar-android/tree/master/prebuilt'
echo '  proot-android:  https://github.com/green-green-avk/build-proot-android/tree/master/packages'
echo '  run-tpl:  https://github.com/green-green-avk/AnotherTerm-scripts/blob/master/assets/run-tpl'
echo
echo 'minitar , proot-android-*Arch'"$VARIANT"'.tar.gz , run-tpl  (put in) directory:'
...
"$TERMSH" copy $OO -fp \
"$SHARED_DATA_DIR/proot-android-$ARCH$VARIANT.tar.gz" \
| "$MINITAR"
...
"$TERMSH" copy $OO -fp "$PRFS" | "$MINITAR"
...
echo 'creating: etc/resolv.conf_off'
#rm -r etc/resolv.conf 2>/dev/null || true # Ubuntu Focal
cat << EOF > etc/resolv.conf_off
nameserver 8.8.8.8
nameserver 8.8.4.4
EOF

echo 'creating: etc/hosts_off'
cat << EOF > etc/hosts_off
127.0.0.1 localhost
EOF
